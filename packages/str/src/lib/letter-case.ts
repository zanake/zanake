...
export const toLabel = (str?: string | Array<string>): string | never => {
    ...
    if (!_str && !_arr && !date)
        throw new TypeError(`Expected a date, string, array of strings, given ${inspect(str)}`);
...
export const toCamel = (value: string, locale = false): string => {
    return (value.match(/[A-Z]{2,}(?=[A-Z][a-z]+[0-9]*|\b)|[A-Z]?[a-z]+[0-9]*|[A-Z]|[0-9]+/g) ?? []).reduce(
        (acc: string, val: string, idx: number): string => {
            return idx !== 0
...
export const toStart = (value: string, locale = false): string => {
    return value.split(/\s/g).reduce((acc, val) => {
        return acc + '\u00A0' + `${toUpper(val.charAt(0), locale)}${toLower(val.slice(1), locale)}`;
    }, '');
};
